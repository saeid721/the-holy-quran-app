...
class ParaIndexScreen extends StatefulWidget {
  const ParaIndexScreen({Key? key}) : super(key: key);

  @override
  State<ParaIndexScreen> createState() => _ParaIndexScreenState();
}

class _ParaIndexScreenState extends State<ParaIndexScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final juzCubit = ParaCubit.cubit(context);
          ...
          children: <Widget>[
                  ...
                  if (value.isNotEmpty) {
                    if (value.length <= 2) {
                      setState(() {
                        ...
                        if (_searchedIndex <= ParaUtils.paraNames.length &&
                            _searchedIndex >= 0) {
                          _searchedJuzName =
                              ParaUtils.paraNames[_searchedIndex - 1];
                        }
                      });
                    }
                  }
            ...
            Container(
              ...
              child: hasSearched
                  ? GestureDetector(
                      onTap: () async {
                        await juzCubit.fetch(
                          ParaUtils.paraNames.indexOf(_searchedJuzName) + 1,
                        );
                  ...
                  : GridView.builder(
                      itemCount: ParaUtils.paraNames.length,
                      gridDelegate:
